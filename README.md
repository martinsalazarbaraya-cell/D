...
La sección **Registro** incrusta un Microsoft Form con un `<iframe>`. Las respuestas se ven en Forms → **Respuestas → Abrir en Excel**.

- Para cambiar de formulario: en Forms, **Compartir → Insertar**, y reemplaza el `src` del `<iframe>` y el link "Ábrelo en una pestaña nueva" en `index.html`.
- Para que respondan invitados externos, el Form debe estar en **"Cualquiera puede responder"**.
- El aspecto de adentro del formulario se ajusta en Forms → **Estilo**.
